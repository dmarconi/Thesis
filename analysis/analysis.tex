...
The following chapter the analysis with two same sign \hadtau and two jets performed with 8\tev data. After a brief description on the data and Monte Carlo samples, details follow on the selection choices used following the strategy suggested in Chapter \ref{sec:VBFSUSY}. The most important section motivates and describes the data-driven background estimation technique used for treating QCD background events where two jets get mis-reconstructed as \hadtau and pass the event selection. The validation of the assumption that has been made while describing the background estimation method is described in the final section.
