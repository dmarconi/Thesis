...
\begin{table}[ht]
	\centering{
		\begin{tabular}{| l |}
			\hline
			\\
			\texttt{CMSSW collection label: TriggerResults\_HLT}  \\
			\\
			\texttt{CMSSW type: edm::TriggerResults} \\
	...
	\label{teble:triggers} % is used to refer this table in the text
\end{table}
...
All the collision data events passing the requirements of the triggers shown on Table \ref{teble:triggers} are considered as the interesting events for offline analysis.
