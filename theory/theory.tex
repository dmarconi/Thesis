...
The superpotential of the MSSM does not cause
electroweak symmetry breaking; this symmetry breaking is only caused by the soft SUSY-breaking terms  \cite{Martin:1997ns}. Taking kinetic mass terms together with the MSSM-potential from Equation \ref{eq::mssmpotential} and the soft breaking terms, one obtains the soft MSSM MSSM Lagrangian $\mathcal{L}_{\mathrm{soft}}^{\mathrm{MSSM}}$~\cite[p56]{martinsusyprimer97}:
\begin{eqnarray}
...
Here the Yukawa couplings $\bf{y_f}$ have been replaced by the dimensionful mass matrices $\bf{a_f}$. The parameters $M_1$, $M_2$, and $M_3$ are the bino, wino, and gluino mass terms. If a soft supersymmetry breaking scale of $m_{\mathrm{soft}}$ is assumed, one should have~\cite[p56]{martinsusyprimer97}
...
\end{eqnarray}
where it was thought that $m_{\mathrm{soft}}$ should not be much larger than 1000 GeV~\cite[p56]{martinsusyprimer97}.


This model results in 105 masses, phases, and mixing angles that cannot be rotated away by redefinitions, nor have a counterpart in the Standard Model [62]. Several models have been developed based on certain assumptions in order to reduce the number of parameters
...
It results in pair production of supersymmetric particles at colliders and a lightest stable
particle that is also a candidate for dark matter
